...
# seperate data to sucking and weaned stage
...
#model, optimizer, epoch, loss = load_checkpoint(model, optimizer, '*.pt')
    ...
    with torch.no_grad():
        for sow_data1, piglet_data, masks1 in dataloader:
            ...
            for feature_index in range(feature_size):
                ...
                r2 = r2_score(true_feature_values, predicted_feature_values)  
    ...
    with torch.no_grad():
        ...
        for pig_id in tqdm(pig_ids, desc="Calculating coreness"):
            ...
            for removed_feature in feature_names:
                ...
                # 获取相对丰度并计算差异                for stage in stages:
